...
MAIN : {
    my ($read1_bam, $read2_bam, $mq) = @ARGV;

	if ( (! defined($read1_bam)) || (! defined($read2_bam)) ){
        die ("Usage: ./two_read_bam_combiner_fifo.pl <read 1 bam> <read 2 bam> <minimum map quality filter>\n");
    }

    open(FILE1, '<', $read1_bam);
    open(FILE2, '<', $read2_bam);
    ...
    while (defined($line1)){    # modified to also print other header lines than @SQ.
        if ($line1 =~ /^\@/){
            if ($line1 =~ /^\@SQ/){
                if ($line1 ne $line2){
                    print($line1);
                ...
                if ($line1 ne $line2){
                    print($line1);
                    print($line2);
                    die ("Inconsistent read group headers. Reads must have same read group.");
                }
                print($line1);
            }
            elsif ($line1 =~ /^\@PG/){    # duplicated @PG lines cause problems in the MergeSamFiles step.
#                print($line1);
#                print($line2);
            }
            else {
